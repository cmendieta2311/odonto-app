...
@Component({
  ...
  template: `
    <div class="bg-surface-light dark:bg-surface-dark rounded-xl border border-[#dbe4e6] dark:border-[#2a3e42] shadow-sm overflow-hidden flex flex-col h-full">
      <div class="overflow-x-auto flex-1">
        <table class="w-full text-left border-collapse">
          ...
          <tbody class="divide-y divide-[#dbe4e6] dark:divide-[#2a3e42]">
            <tr *ngIf="isLoading">
              <td [attr.colspan]="columns.length + (hasActions ? 1 : 0)" class="p-0">
      ...
      <!-- Pagination -->
      <div *ngIf="showPagination && (totalItems > 0 || isLoading)" class="p-4 border-t border-[#dbe4e6] dark:border-[#2a3e42] flex flex-col sm:flex-row items-center justify-between gap-4">
        <div class="flex items-center gap-4">
          <div class="flex items-center gap-2">
            ...
            <select 
              ...
              [disabled]="isLoading"
              class="bg-background-light dark:bg-background-dark border border-[#dbe4e6] dark:border-[#2a3e42] text-text-main-light dark:text-text-main-dark text-xs rounded-lg px-2 py-1 outline-none cursor-pointer focus:border-primary transition-colors disabled:opacity-50">
